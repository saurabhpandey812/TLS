...
const sendFollowRequest = async (req, res) => {
  try {
    ...
    const result = await sendFollowRequestService({ userId: req.user.userId, targetUserId: req.params.targetUserId, user: req.user, io: req.app.get('io') });
...
const acceptFollowRequest = async (req, res) => {
  try {
    ...
    const result = await acceptFollowRequestService({ userId: req.user.userId, followerId: req.params.followerId, user: req.user, io: req.app.get('io') });
    if (!result.success) return res.status(400).json(result);
    return res.status(200).json(result);
  } catch (error) {
...
const rejectFollowRequest = async (req, res) => {
  try {
    ...
    const result = await rejectFollowRequestService({ userId: req.user.userId, followerId: req.params.followerId, user: req.user, io: req.app.get('io') });
    if (!result.success) return res.status(400).json(result);
    return res.status(200).json(result);
  } catch (error) {
